...
public class GeoUtils {
     ...
     */
    public static boolean isPointInPolygon(Point2D point, List<Point2D> polygon) {
        ...
        for (int i = 0, j = polygon.size() - 1; i < polygon.size(); j = i++) {
            p0 = polygon.get(j);
            p1 = polygon.get(i);
            // 判断point是否就是顶点本身，这里不增加判断与p1是否相等，可避免重复判断，下一次就会判断是否在p1上,缺点是会多判断一次边
            if (point.equals(p0)) {
                return true;
            }
            // 如果相交，则记录相交次数（通过奇数、偶数来设置isIn的值）
     ...
     */
    private static boolean isIntersectWithLine(Point2D p, Point2D p0, Point2D p1) {
